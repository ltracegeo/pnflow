...
void Polygon::insertWatSnapEvent_IfSnapPcHgPc(Events<Apex*,PceImbCmp>& watEvents,  double globalPc)  {
...
void VoidElem::setRadius(double newRad)  {
	R_ = newRad;
	area_ = pow(newRad, 2.) / (4. * shapeFactor_);
	ensure(area_ > 0.);
}
...
void VoidElem::setContactAngle(double refCAng, int wettClass, double DAng)  {
	...
	if(wettClass == 1)  {
		...
		cntAngRec_ = max(0., growthExp*refCAng-DAng);
		cntAngAdv_ = min(PI, growthExp*refCAng);
	}
	else  if(wettClass == 3)  {
		if     (refCAng < 0.38349) cntAngRec_ = 0.;
		else if(refCAng < 1.5289) cntAngRec_ = (0.5*exp(0.05*refCAng*180./PI)-1.5)*PI/180.;
		else if(refCAng < 2.7646) cntAngRec_ = 2.*(refCAng-1.19680);
		else cntAngRec_ = PI;

		if     (refCAng < 0.38349) cntAngAdv_ = 0.;
		else if(refCAng < 1.61268) cntAngAdv_ = 2.*(refCAng-0.38349);
		else if(refCAng < 2.75805) cntAngAdv_ = (181.5 - 4051.*exp(-0.05*refCAng*180./PI))*PI/180.;
		else                       cntAngAdv_ = PI;
	}
	else  if(wettClass == 4)  {
		cntAngAdv_ = refCAng;
		cntAngRec_ = pow(PI-1.3834263-pow(PI-cntAngAdv_+0.004,0.45), 1./0.45)-0.004;
	}
	else
	{
		double PlusCoef = PI-(0.1171859 *DAng*DAng*DAng -0.6614868 *DAng*DAng + 1.632065 *DAng) ;
		double powrCoef = 1.-(0.01502745 *DAng*DAng*DAng -0.1015349 *DAng*DAng + 0.4734059 *DAng) ;

		cntAngAdv_ = refCAng;
		cntAngRec_ = pow(PlusCoef-pow(PI-cntAngAdv_+0.004,powrCoef), 1./powrCoef)-0.004;
	}
	...
	//Pc_pistonTypeAdv_ = comn_.sigmaOW()*(2.*cos(cntAngAdv_)) / R_;     // TOBE initialised properly later
	//Pc_pistonTypeRec_ = comn_.sigmaOW()*(2.*cos(cntAngRec_)) / R_;     // TOBE initialised properly later
...
void Triangle::setShapeFactor(double shapeFact)  {
	ensure(shapeFact <=  sqrt(3.)/36.);
	shapeFactor_ = max(shapeFact,1e-6);
	area_ = pow(R_, 2.) / (4. * shapeFactor_);
	ensure(area_ > 0.);
	//areaWater_ = area_;
	setHalfAngles();
...
void Triangle::setHalfAngles()  {
	double beta_2_min = atan((2./sqrt(3.))*cos(acos(-12.*sqrt(3.)*shapeFactor_)/3.+4.*PI/3.));
	double beta_2_max = atan((2./sqrt(3.))*cos(acos(-12.*sqrt(3.)*shapeFactor_)/3.));
	double randNum = double(rand()) / double(RAND_MAX);
	randNum = 0.5*(randNum+0.5);///.25-.75

	crnHafAngs_[1] = beta_2_min + (beta_2_max - beta_2_min)*randNum;
	crnHafAngs_[0] = 0.5*(asin((tan(crnHafAngs_[1])+4.*shapeFactor_)
		* sin(crnHafAngs_[1]) / (tan(crnHafAngs_[1])-4.*shapeFactor_))-crnHafAngs_[1]);
	crnHafAngs_[2] = PI/2. - crnHafAngs_[1] - crnHafAngs_[0];
}
...
*/
Circle::Circle(Elem& parent, const CommonData& common, double radius, int connNum, int)
	  : VoidElem(parent, common, radius, 1./(4.*PI), connNum)  {
}
...
void Polygon::calcOilLayerPc_syncTrappings(double pc)  {///. pc is used for error checking only
	...
	if(watTrpBulk.first > -1)  {
		...
		for(int i = 0; i < numCorners_; ++i)  {
			...
			if (!oilLayer_[i].finitLayerApex(watTrpBulk.second, cntAngRec_, cntAngAdv_, crnHafAngs_[i],tension, oilInj, true))  {
				...
				if(!(oilLayer_[i].pinnedApexDist() < (R_*(1./tan(crnHafAngs_[0])+1./tan(crnHafAngs_[1])))))  {
					...
					cout<<oilLayer_[i].pinnedApexDist() <<"  <  "<< R_<<" * (1./tan( "<<crnHafAngs_[0]<<" ) + 1./tan( "<<crnHafAngs_[1]<<endl;
					cout<<oilLayer_[i].pinnedApexDist() <<"  <  "<< (R_*(1./tan(crnHafAngs_[0])+1./tan(crnHafAngs_[1])))<<endl;
					//((double*)&R_)[100000]=0.;
...
void Polygon::calcOilLayerPc_markUntrappedFilms(double pc)  {///. pc is used for error checking only
...
{
	double snapOffPrs;
	if(cntAngRec_ > PI/2. + crnHafAngs_[0])      // Spontaneous
	{
...
//not converging
	...
	for(int i = 0; i < MAX_NEWT_ITR; ++i)               // Larger contact angles  => Need to use a NR technique
	{
		...
		double newPc = (abs(funcDpc)>1e-32) ? oldPc - func / funcDpc  : oldPc - func;
		if (i>MAX_NEWT_ITR/2 ) newPc = 0.5*(newPc+oldPc);

		errorVal = fabs(newPc-oldPc)/(abs(oldPc)+1e-8);
		if(errorVal < EPSILON)  {
...
/*    double tension(comn_.sigmaOW());
	double snapOffPrsOne = (tension/R_) * (cos(cntAngAdv_) -
			(2.*sin(cntAngAdv_))/(1./tan(crnHafAngs_[0]) + 1./tan(crnHafAngs_[1])));    // Small Contact Ang
	...
	for(int i = 0; i < MAX_NEWT_ITR; ++i)               // Larger contact angles  => Need to use a NR technique
	{
		...
		double func = oldPc - (comn_.sigmaOW()/R_)*((cos(cntAngAdv_)/tan(crnHafAngs_[0]) -
			...
			(1./tan(crnHafAngs_[0]) + 1./tan(crnHafAngs_[2])));

		double funcDpc = 1 + (comn_.sigmaOW()/R_)*((hingAngDpc*sin(hingAng)/tan(crnHafAngs_[2]) +
			hingAngDpc*cos(hingAng)) / (1./tan(crnHafAngs_[0]) + 1./tan(crnHafAngs_[2])));
...
double Triangle::snapOffPrsDrainFromCorner(bool& casePossible, int cor) const
{
	...
	double tension(comn_.sigmaOW()), errorVal(1.);
	...
	while(!initGuesses.empty())  {
		...
		for(int i = 0; i < MAX_NEWT_ITR; ++i)               // Larger contact angles  => Need to use a NR technique
		{
			...
			double func = oldPc - (tension/R_)*(
				...
				(1./tan(crnHafAngs_[0]) + 1./tan(crnHafAngs_[cor]))
				);

			double funcDpc = 1 - (tension/R_)*((tetaCorDpc*sin(tetaCor)/tan(crnHafAngs_[cor]) -
				tetaCorDpc*cos(tetaCor)) / (1./tan(crnHafAngs_[0]) + 1./tan(crnHafAngs_[cor])));

			double newPc = oldPc - func / funcDpc;
			errorVal = fabs((newPc - oldPc)/oldPc);

			if(errorVal < EPSILON)  {
				dbgAsrt(tetaCor >= cntAngRec_);
...
double Triangle::snapOffPrsDrainSpontNR() const
{
	...
	/(sin(crnHafAngs_[0])* R_*(1./tan(crnHafAngs_[0])+1./tan(crnHafAngs_[2])));

	double caseTwoSnapOff = (tension/R_) * (cos(cntAngRec_) +
			(2.*sin(cntAngRec_))/(1./tan(crnHafAngs_[0]) + 1./tan(crnHafAngs_[1])));
	...
	if(case3Possible && case4Possible)
		...
		return min(caseFourSnapOff, min(caseTwoSnapOff, caseOneSnapOff));
	else if(oilLayer_[1].exists() && cntAngRec_ > PI/2. + crnHafAngs_[0] )  //|| trappedCL_.first < 0;
		return min(caseTwoSnapOff, caseOneSnapOff);
	else
		return caseOneSnapOff;
}
